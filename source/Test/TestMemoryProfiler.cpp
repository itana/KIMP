...
{
	MemoryProfiler& mem_profiler = MemoryProfiler::singleton();
	mem_profiler.setEnable(true);

	mem_profiler.reset();
	{
		//MemoryProfiler::Scope s("test");
		void *p = malloc(10240);
		free(p);
	}

	std::string s = mem_profiler.defaultReport(20, 0);
